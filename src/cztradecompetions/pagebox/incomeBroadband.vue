<template>
  <div class="income_trend_box" >
    ...
    <div class="button_group">
      <Button-group>
            <i-button :class="{'ivu-btn-primary':mb5g_nav === 'income'}" @click="mb5g_nav = 'income'">宽带收入</i-button>
            <i-button :class="{'ivu-btn-primary':mb5g_nav === 'user'}" @click="mb5g_nav = 'user'">宽带用户</i-button>
            <i-button :class="{'ivu-btn-primary':mb5g_nav === 'flow'}" @click="mb5g_nav = 'flow'">宽带电视</i-button>
      </Button-group>
    </div>
    <div class="chat_box">
  ...
  methods: {
    async getLeftTopData(){
      ...
      if(this.mb5g_nav == 'income'){
          encodesUserPart = ['HYJZZB0132','HYJZZB0131','HYJZZB0130'];
      }
      if(this.mb5g_nav == 'user'){
          encodesUserPart = ['HYJZZB0141','HYJZZB0140','HYJZZB0139'];
      }
      if(this.mb5g_nav == 'flow'){
          encodesUserPart = ['HYJZZB0150','HYJZZB0149','HYJZZB0148'];
      }
    ...
    async getRiseRateData(){
      ...
      const encodeRiseRate = ['HYJZZB0135','HYJZZB0134','HYJZZB0133'];
      const encodeIncomeNum = ['HYJZZB0138','HYJZZB0137','HYJZZB0136'];
      //user
      const increasedUserMonth = getMonthsArr(year,month,16);
      const encodesNewIncreasedUser = ['HYJZZB0147','HYJZZB0146','HYJZZB0145'];
      const encodesGrowthUser = ['HYJZZB0144','HYJZZB0143','HYJZZB0142'];
      //flow
      const flowMonthArr = increasedUserMonth;
      const encodeMbFlow = ['HYJZZB0153','HYJZZB0152','HYJZZB0151'];
      const encodeFlowIncome = ['HYJZZB0156','HYJZZB0155','HYJZZB0154'];
      const encodeFlowUintPrice= ['HYJZZB0159','HYJZZB0158','HYJZZB0157'];
    ...
    setCharts(){
      this.getLeftTopData().then(d => {
          ...
          if(this.mb5g_nav == 'income'){
               f.title.text = '宽带收入额走势（累计）';
          }
          if(this.mb5g_nav == 'user'){
              f.title.text = '宽带用户份额';
          }
          if(this.mb5g_nav == 'flow'){
              f.title.text = '宽带电视用户份额';
          }
      ...
      this.getRiseRateData().then(d => {
        ...
        const f_lt = JSON.parse(JSON.stringify(incomeTrendRateOfRise));
          ...
          if(this.mb5g_nav == 'flow'){
            ...
            const unitflow_arr = searchCodeMonthsValArr(d.unitflowencode, code, montsArr, d.data,2);
            cf_flow.title.text = '宽带电视用户'
            cf_flowic.title.text = '宽带电视收入'
            cf_flowunit.title.text = '宽带电视ARPU（元/月）'
